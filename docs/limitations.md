...
[실제 Claude 평가](hosted-evaluation-success.md)는 합성 10건 1회 실행이다. 단독 추출은
18/30, gated는 30/30이지만 정상 필드는 모두 로컬 규칙 결과다. 같은 셋의 순수 규칙도
30/30이므로 LLM 추가 효용은 미증명이다. 비용은 provider가 보고하지 않아 미측정이다.
