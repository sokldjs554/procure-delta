...
[최신 Claude v3 평가](hosted-evaluation-success.md)는 단독 추출 30/30과 측정 검증을 통과했다.
이전 v2는 18/30이었다. 각 버전 1회 합성 회귀이며 실제 문서 일반화나 반복성을 뜻하지 않는다.
...
15,654→7,317을 확인했다. 이 [v2 기록](hosted-evaluation-v2.md)은 보존했다.
15:00 UTC에 시작한 v3 전체 평가도 통과했고 호출 10→5회, 토큰 22,343→10,149를 기록했다.
두 요청 버전의 정확도·지연 변화와 한계는 [최신 비교 보고서](hosted-evaluation-success.md)에 있다.
...
4. `scripts/validate_hosted_eval.py`의 통과 여부와 개별 사례 오류를 확인한다.
   ...
   구분하고 사례별 토큰과 합계를 확인한다. 새 실행에는 `grounding_issues`도 기록한다.
   이 목록은 필드명과 정해진 거절 코드만 포함하며 제안 값·인용·응답 원문은 포함하지
   않는다. 과거 artifact에 없던 사유를 추정해서 채우지 않는다.
   측정 validator 통과와 추출 품질 합격을
...
### Grounding 요청 보완 후 재평가

`chat-*-v3` 요청은 기존 검증기의 한국어 날짜 시간대, 원문 인용, 선택 조건 생략,
필수 필드 부재·모순 시 거절 규칙을 명시한다. 실제 프롬프트와 스키마가 요청 식별자에
포함돼 이전 결과를 재사용하지 않는다. [변경 범위](hosted-grounding-contract.md)와
로컬 회귀 검증과 별도로 [v3 외부 실행 36016974477](hosted-evaluation-success.md)을 완료했다.
정상 5건은 30/30, 거절 대상은 5/5 차단됐지만 토큰과 단독 경로 p95는 v2보다 늘었다.
과거 원본을 보존하고 최신 결과만 공개 projection에 반영했다. 새 요청 버전의 같은
회귀셋 1회 관측이며, 이후 반복 실행과 실문서 평가는 별도로 필요하다.
