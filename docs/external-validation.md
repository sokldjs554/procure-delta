...
로컬 회귀 검증은 **새 Claude 정확도 실측이 아니다**. 기존 데이터셋·검증 기준·공개
실측 artifact는 유지한다. 다음 일반 workflow 실행에서 정상 5건의 필드 정확도와
거절 사유, 거절 5건의 결과, 전체 사용량을 함께 비교한다. 프롬프트가 길어졌으므로
과거 토큰 절감률도 새 요청의 수치로 간주하지 않는다.
