# Claude v2 외부 실측 기록: 측정 완료와 품질 한계

이 문서는 v2 실행 당시의 기록이다. [최신 v3 결과와 비교](hosted-evaluation-success.md)는 별도 문서에 있다.
...
- [변경하지 않은 hosted.json](../artifacts/evaluation/history/claude-35949777404/hosted.json)
- [workflow에서 생성한 검증 요약](../artifacts/evaluation/history/claude-35949777404/hosted-summary.json)
...
```sh
python scripts/validate_hosted_eval.py artifacts/evaluation/history/claude-35949777404/hosted.json
```

위 명령은 보존된 v2 결과만 검증하며 외부 모델을 다시 호출하거나 현재 v3 화면을 변경하지 않는다.
