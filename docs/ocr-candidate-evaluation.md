...
아래 수치는 2026-09-24의 서식 해석 v2 기준이다. 이후 같은 인식 텍스트에 대한
[서식 해석 v3 재측정](korean-form-spans.md)에서 여주 개별 사례는 검증 통과 정답
2/3이 됐다. 실패가 있는 그룹은 계속 `null`이고 후보의 운영 채택은 보류한다.
...
이 문서의 v2 실험에서는 구조화 검증 통과가 개선되지 않았다. 후속 v3에서도
원본 스캔의 검증 통과 정답은 0/6이며 자원 한도 내에서 6건 모두 완료하지 못했다.
