...
선택적으로 [hosted 추출 워커와 원장을 연결](extraction-credits.md)할 수 있습니다.
공유 수집 작업은 배포 설정에 명시한 파이프라인 예산 계정을 사용하며, 고객 계정을
임의로 선택하지 않습니다. 호출 전 예약을 영속화하고 결과와 차감을 함께 확정합니다.
호출·저장 결과가 불명확하면 예약을 유지하고 자동 재호출·환불을 막습니다.
운영자 CLI에서 예산 지급·조회·확인 후 정산을 할 수 있습니다.
고정 내부 단위이며 실제 provider 비용, 고객별 과금, 정기결제나 OCR 과금은 아닙니다.
