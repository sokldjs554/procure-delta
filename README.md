...
- [공식 사이트의 원본 스캔 2건](docs/public-scan-ocr.md)은 워커 어댑터의 페이지 간 메모리 문제를 수정한 뒤 두 번 모두 8쪽 처리를 완료했습니다. 같은 기관·유사 서식의 작은 표본이며, 첫 페이지의 검증 통과 필드는 **0/6**입니다. 전체 PDF 추출 승인·DB 저장·클라우드 성능·나라장터 첨부 수집 실측을 뜻하지 않습니다.
