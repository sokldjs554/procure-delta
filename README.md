...
| 한국어 공고 서식 해석 v3 | 반복 실측 · 제한적 개선 | 같은 후보 OCR 텍스트에서 여주 개별 사례의 검증 통과 정답 **0/3 → 2/3**. 실패 1건이 있는 그룹 점수는 `null`, 원본 스캔은 **0/6** 유지; [근거·한계·재측정](docs/korean-form-spans.md) |
...
- [서식 해석 v3 재측정](docs/korean-form-spans.md)에서는 여주 한 사례의 검증 통과 정답이 2/3으로 늘었지만 제목 공백 오류는 남았습니다. 검증기는 OCR 텍스트와의 근거 일치를 확인하므로 인식 오류까지 보증하지 않습니다. 같은 여섯 문서로 개발·반복 검증한 결과이며 신규 독립 표본의 성능이 아닙니다.
